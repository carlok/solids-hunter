...
5. **Loop (while `gameActive`):** player WASD on XZ; **ray shoot** on LMB; update entities (motion + wall slide for `slide`); **pairwise XZ separation** (`xzOverlapSeparation`); **`separateEntityFromWalls`** push-out along smallest penetration axis; clamp XZ to `entXZLim = min(29.8, envSpawnHalfXZ - 1.2)` (or re-tune with Babylon scale). **HUD:** crosshair, score, rule badge, hint text, **sound toggle** (must remain clickable with pointer lock — same pattern: isolated `pointer-events: auto` control).
